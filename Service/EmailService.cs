        ...
        public void SendFormReviewEmail(User recieveEmp , string formId)
        {
            ...
            if(recieveFormRecordEmail.ResultId == "RS4")
            {
                ...
                if (isFirstProcessNodeId)
                {
                    ...
                    var btnText = "登入系統";
                    var reviewText = "您好, 提醒您有尚須修改的工單需處理 ! 請至查詢工單頁面<br>目前狀態為**" + resultDescription + "**<br>工單流程進度為" + recieveFormRecordEmail.UserActivityIdDescription.ToString() + "<br>以此溫韾提醒~ 感謝";
                    string SmtpServer = "smtp.gmail.com";
                    int SmtpPort = 587;
                    MailMessage mms = new MailMessage();
                    mms.From = new MailAddress(GoogleID);
                ...
                {
                    //var Url = "https://localhost:7129/Login/LoginAndRedirect?returnUrl=/FormReview/Index?id=" + formId.ToString();
                    var Url = "https://localhost:7129/Login/Index";
                    var btnText = "登入系統";
